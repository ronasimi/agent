...
def run_durable_compute_job(job_id: str, worker_id: str) -> None:
    ...
    if not state:
        state = initialize_state(program, str(payload.get("input_text") or ""))
    ...
    result = run_quantum(program, state, quantum=quantum)
    ...
    if max_tape_cells is not None and int(result.state.get("tape_cells", 0)) > int(max_tape_cells):
        ...
        checkpoint_and_fail_job(job_id, limited, limited["last_error"], step=generation)
        return
    if max_wall_time is not None and _age_seconds(job.get("created_at") or "") >= float(max_wall_time):
        limited = dict(result.state)
        limited["status"] = "failed"
    ...
    if result.status == "halted":
        checkpoint_and_complete_job(job_id, result.state, _terminal_summary(result.state), step=generation)
        return
    if result.status == "failed":
        checkpoint_and_fail_job(job_id, result.state, result.error or "deterministic machine failed", step=generation)
        return
