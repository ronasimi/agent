...
FAST_MODEL = str(AGENT_CFG.get("fast_model") or EXECUTOR_MODEL)
VISION_MODEL = str(AGENT_CFG.get("vision_model") or "qwen3.5:4b")
VISION_MODEL_KEEP_ALIVE = AGENT_CFG.get("vision_model_keep_alive", "2m")
...
REPORT_MODEL = AGENT_CFG.get("report_model", REASONING_MODEL or MODEL)
REPORT_MODEL_KEEP_ALIVE = AGENT_CFG.get("report_model_keep_alive", "10m")
MAIN_OPTIONS = AGENT_CFG.get("main_options", {"num_ctx": 16384, "temperature": 0.3})
FAST_OPTIONS = AGENT_CFG.get("fast_options", dict(MAIN_OPTIONS))
DECISION_OPTIONS = AGENT_CFG.get("decision_options", {"num_ctx": 8192, "temperature": 0.0, "num_predict": 128})
REASONING_OPTIONS = AGENT_CFG.get("reasoning_options", {"num_ctx": 16384, "temperature": 0.4})
VISION_OPTIONS = AGENT_CFG.get("vision_options") or (dict(REASONING_OPTIONS) if VISION_MODEL == REASONING_MODEL else dict(MAIN_OPTIONS))
