...
The checkpoint format is versioned independently from the program. It persists the current machine state, head position, exact transition count, yield count, checkpoint generation, populated-cell count, and bounded observability fields. Tape cells are stored separately in an arbitrary-precision paged table, `durable_compute_tape(job_id, page_index, cell_offset, symbol)`, rather than copied into every checkpoint. `page_index` is a decimal `TEXT` key and `cell_offset` is bounded to a 4096-cell page, so persisted tape addresses are not limited by SQLite's signed-64-bit `INTEGER` range. Unknown checkpoint versions are rejected rather than silently reset.
