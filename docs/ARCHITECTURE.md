...
tools.config.load_config normalizes every inference consumer to agent.model and agent.main_options. The default model alias is agent-main:4b, backed by the original distilled 4B GGUF. Compatibility constants named FAST_MODEL, REPORT_MODEL, or COMPACTION_MODEL all refer to this same model. There is no specialist model escalation, embedding runner, vision sidecar, or model-residency swap.

The default protocol is schema-constrained JSON. Each response is exactly one of:

```json
{"action": "tool", "name": "calculate", "arguments": {"expression": "19 * 23"}}
```
...
The action schema incorporates the currently loaded functions' argument schemas. The native protocol uses the same loop and registry with Ollama tools and tool-result messages. Protocol selection is an operator setting, independent of task content. A malformed response never triggers a switch to another model or protocol.
...
1. The complete model stream must arrive before any selected action executes. Incomplete responses, token-limit truncation, prose-shaped calls, and malformed JSON are rejected.
2. JSON duplicate keys, non-finite values, missing parameters, wrong types, and unexpected top-level arguments are rejected. The harness does not invent parameters.
3. Native batches are validated before execution. Calls run sequentially, and a failure skips later calls in that batch. Dependencies across tool results should use separate model turns.
...
Storage keeps native assistant/tool roles even in JSON mode. Conversion to text-model action and observation messages occurs only at the provider boundary, preventing protocol details from appearing as fake saved user messages.
...
The default text-only model cannot inspect pixels. Autonomous selection also does not guarantee that a 4B model will choose the right tool or produce a correct answer. The live smoke test and opt-in model tests must be run on the target Ollama deployment.
