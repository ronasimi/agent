...
`diagnostics/simulate_turns.py` replaces the Ollama transport with a scripted client
...
| --- | --- | --- |
| Working state and evidence digest were emitted at message index 1, before the history | Reusable prefix collapsed to **0.1%** whenever the state changed; one multi-check turn re-prefilled 11,374 of 11,382 chars on iteration 2 | Historical optimization: `volatile_blocks_last` moved working state after history. This is now deprecated because strict Qwen/Ollama templates reject non-leading system messages; working state is merged into the single leading system role instead. |
