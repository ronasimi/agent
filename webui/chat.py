...
def _should_forward_event(
    event: dict[str, Any], *, thinking_enabled: bool = False
) -> bool:
...
async def _run_turn(websocket: WebSocket, payload: dict[str, Any]) -> None:
    ...
    if not text:
        await websocket.send_json(
            {"type": "error", "turn_id": turn_id, "message": "Message is empty"}
        )
        return
        ...
        await websocket.send_json(
            {
                "type": "error",
                "turn_id": turn_id,
                "message": "Turn ID is already active",
            }
        )
        return
    ...
    try:
        ...
        def work() -> None:
            ...
            messages = [
                {"role": "system", "content": agent_runtime.build_system_prompt()}
            ]
            with (
                conversation_context(conversation_id),
                agent_runtime.frontend_event_context(sink, cancel_event),
            ):
        ...
        # This also covers failure while acknowledging or scanning attachments,
        # before the worker task and its inner cleanup scope exist.
        with RUNS_LOCK:
            if RUNS.get(turn_id) is cancel_event:
                RUNS.pop(turn_id, None)
            ...
            if active and not active.done():
                if bool(payload.get("defer_until_idle", False)):
                    ...
                    await websocket.send_json(
                        {
                            "type": "error",
                            "message": "This browser session already has an active turn",
                        }
                    )
                    continue
