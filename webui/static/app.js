...
function jobProgress(j){if(j.job_type!=='durable_compute'||!j.progress)return '';const p=j.progress;return `<div class="job-progress"><span>${mdiIcon('counter','↻')} ${Number(p.steps||0).toLocaleString()} steps</span><span>${mdiIcon('content-save-check-outline','◇')} ${Number(p.yield_count||0).toLocaleString()} yields</span><span>${mdiIcon('database-outline','▦')} ${Number(p.tape_cells||0).toLocaleString()} cells</span><span>${mdiIcon('state-machine','◎')} ${esc(p.machine_state||p.machine_status||'not started')}</span></div>`;}
