...
function addValidator(e){const el=document.createElement('div');el.className='validator';const recipe=e.suggested_recipe?` · recipe: ${e.suggested_recipe}`:'';el.textContent=`Fast validator: ${e.decision||'—'}${e.diagnosis?' · '+e.diagnosis:''}${e.suggested_tool?' · '+e.suggested_tool:''}${recipe}`;messagesEl.appendChild(el);scrollBottom();}
